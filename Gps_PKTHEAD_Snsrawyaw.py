...
combined_dfs = []
mc_name_li = []
last_PKTHEAD_values = []  # 存储每个GNRMC记录对应的最后一个PKTHEAD值
...
for f in os.scandir(DATA_DIR):
    if f.path.endswith('.mc'):
        ...
        df_GNRMC['last_PKTHEAD_value'] = pd.NA  # 用 pd.NA 初始化新列
        ...
        if not df_GNRMC.empty:
            ...
            df_PKTHEAD = df_mc[df_mc[0] == '$PKTHEAD'].copy()
            df_PKTHEAD = df_PKTHEAD.iloc[:, :15]
            df_PKTHEAD['can_raw_yaw_rate'] = .0

            # 检查是否有PKTHEAD数据并且GNRMC的速度值不是缺失值
            if not df_PKTHEAD.empty and first_gnrmc_speed is not None and not pd.isna(first_gnrmc_speed):
                # 将GNRMC的第一行速度值赋给PKTHEAD的第一行can_raw_yaw_rate列
                df_PKTHEAD.iloc[0, df_PKTHEAD.columns.get_loc('can_raw_yaw_rate')] = first_gnrmc_speed

        # 处理 $PKTHEAD 数据
        for i in df_PKTHEAD.index:
            if i == df_PKTHEAD.index[0]: continue
            prev_idx = df_PKTHEAD.index[df_PKTHEAD.index.get_loc(i) - 1]
            # 这里的代码已经为df_PKTHEAD计算了can_raw_yaw_rate
            df_PKTHEAD.loc[i, 'can_raw_yaw_rate'] = float(df_PKTHEAD.loc[i, 8])
        ...
        if not df_GNRMC.empty:
            ...
            if not df_SNSRAWYAW.empty and first_gnrmc_speed is not None and not pd.isna(first_gnrmc_speed):
                # 将GNRMC的第一行速度值赋给PKTHEAD的第一行sns_raw_yaw_rate列
        ...
        # 初始化用于存储最后一个PKTHEAD值和can_raw_yaw_rate的变量
        last_PKTHEAD_value = pd.NA
        last_can_raw_yaw_rate = None  # 新增变量

        # 初始化一个列表来存储所有的can_raw_yaw_rate值
        all_raw_yaw_rates = []

        # 初始化用于存储最后一个PKTHEAD值和sns_raw_yaw_rate的变量
        ...
        for index, row in df_mc.iterrows():
            if row[0] == '$PKTHEAD' and row[1] == '2':
                # 获取当前的can_raw_yaw_rate值，并转换为浮点数
                current_can_raw_yaw_rate = float(
                    df_PKTHEAD.loc[df_PKTHEAD.index == index, 'can_raw_yaw_rate'].values[0])
                ...
                if all_raw_yaw_rates:
                    last_PKTHEAD_value = sum(all_raw_yaw_rates) / len(all_raw_yaw_rates)
                    # if last_PKTHEAD_value > (float(row[8]) + 280.0):
                    #     last_PKTHEAD_value -= 360
                    # if last_PKTHEAD_value < (float(row[8]) - 280.0):
                    #     last_PKTHEAD_value += 360

                    # print(all_raw_yaw_rates)

                    df_GNRMC.loc[index, 'can_raw_yaw_rate'] = last_PKTHEAD_value

                    if all_sns_raw_yaw_rates:
                        last_snsrawyaw_value = sum(all_sns_raw_yaw_rates) / len(all_sns_raw_yaw_rates)
                        if last_snsrawyaw_value > (float(row[8]) + 280.0):
                            last_snsrawyaw_value -= 360
                        if last_snsrawyaw_value < (float(row[8]) - 280.0):
                            last_snsrawyaw_value += 360
                        # print(all_sns_raw_yaw_rates)
                        df_GNRMC.loc[index, 'sns_raw_yaw_rate'] = last_snsrawyaw_value
                    else:
                        last_snsrawyaw_value = None  # 如果没有值，设置为None
                else:
                    last_PKTHEAD_value = None  # 如果没有值，设置为None
                    last_snsrawyaw_value = None

                # 为$GNRMC行设置平均的last_PKTHEAD_value
                df_GNRMC.loc[index, 'last_PKTHEAD_value'] = last_PKTHEAD_value
        ...
        # 按照原顺序合并数据
        combined = pd.concat([df_PKTHEAD, df_SNSRAWYAW, df_GNRMC], axis=0).sort_index()
        combined_dfs.append(combined)

        # 添加整个PKTHEAD的最后值列表
        sns_yaw_values = df_GNRMC['last_PKTHEAD_value'].tolist()
        last_PKTHEAD_values.append(sns_yaw_values)
...
for i, combined_df in enumerate(combined_dfs):
    ...
    sns_yaw_data = combined_df[combined_df[0] == '$PKTHEAD']
    ...
    charts_sheet.write('C1', 'DR')
    charts_sheet.write('D1', 'RAWYAW')

    for j in range(len(time_data)):
        last_PKTHEAD = last_PKTHEAD_values[i][j] if not pd.isna(last_PKTHEAD_values[i][j]) else None
        ...
        charts_sheet.write(j + 1, 2, last_PKTHEAD)  # 最后的PKTHEAD值写入C列
    ...
    # 添加第C列的数据作为新的系列（PKTHEAD值）
    chart.add_series({
        'name': f'DR',
        'categories': f'=charts!$A$2:$A${len(time_data) + 1}',  # 使用相同的X轴数据
        'values': f'=charts!$C$2:$C${len(time_data) + 1}',  # 这里假设时间数据和PKTHEAD数据行数相同
